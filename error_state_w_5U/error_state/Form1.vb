...
Public Class Form1
    Dim add_D1000(1000), add_D101X(1000), add_D1021(1000), add_D14XX(1000), add_R2000(1000), add_R2001(1000), add_R2002(1000), add_R2003(1000), add_R5(1000), add_D1499(1000), add_D1599(1000), add_D1031(1000), add_D1033(1000), add_R1(100), add_D54(100), add_R3(100), add_D58(100) As Int32
    Dim sdb As SqlDB = New SqlDB
    Dim flag As Boolean
    Dim error_cont As Integer
    Dim m_id As String = My.Settings.machine_id
    Dim error_flag As Boolean
    Dim PlcReturnCode As String
    Dim AxActUtlType1 = New AxActUtlTypeLib.AxActUtlType()
    Dim AxActProgType1 = New AxActProgTypeLib.AxActProgType()
    Dim package_len As String = "" '机包长度
    Dim package_push As String = "" '机包压力
    Dim setting_change As Boolean = False            '壓力及長度設定值變更
    Dim package_len_set As Decimal = 0     '171006打包長度
    Dim package_len_real As Decimal = 0     '171006打包長度
    Dim package_push_set As Decimal = 0     '171006擠壓壓力
    Dim package_push_real As Decimal = 0     '171006擠壓壓力
    ...
    Private Sub init()
        ...
        Try
            ...
            iLogicalStationNumber = Convert.ToInt32(My.Settings.PLC_id)
            ...
            PlcReturnCode = iReturnCode
            If PlcReturnCode = "0" Then
        ...
        If flag = True Then
            For a As Integer = 2000 To 2999 Step 10
                ...
                If b = 0 Then
                    ...
                    AxActEasyIF5.ReadDeviceBlock(a0, 1, add_R2000(0), AxActUtlType1)
                    p_date = add_R2000(0)
                    ...
                    AxActEasyIF5.ReadDeviceBlock(a1, 1, add_R2001(0), AxActUtlType1)
                    ...
                    AxActEasyIF5.ReadDeviceBlock(a2, 1, add_R2002(0), AxActUtlType1)
                    Dim ss As String = add_R2002(0)
                    ss = ss.PadLeft(2, "0")
                    Dim p_time As String = hh & ":" & mm & ":" & ss

                    Dim a3 As String = "R" & a + 3.ToString
                    AxActEasyIF5.ReadDeviceBlock(a3, 1, add_R2003(0), AxActUtlType1)
    ...
    Sub ins_state(ByVal m_id As String)
        ...
        Try
            ...
            AxActUtlType1.ReadDeviceBlock2("D1031", 15, add_D1031(0))
            AxActUtlType1.ReadDeviceBlock2("D1033", 15, add_D1033(0))
            package_len = add_D1031(0).ToString()
            package_push = add_D1033(0).ToString()
            'If BinaryBit(auto_m, 2) = 1 Then
            ...
            If chk_ds > 0 Then
                rec_sql = "update package_state SET machine_state = '" & Label3.Text & "', run_time = '" & tot_time & "' ,package_num = '" & tot_num & "',package_len = '" & package_len & "',package_push = '" & package_push & "' where machine_id = '" & m_id & "'  and run_date = CONVERT(varchar(100), GETDATE(), 23) "
                sdb.RunCmd(rec_sql, CommandType.Text)
            Else
                rec_sql = "update package_state SET machine_state = '" & Label3.Text & "', run_time = '-' ,package_num = '0',run_date = CONVERT(varchar(100), GETDATE(), 23),package_len = '" & package_len & "',package_push = '" & package_push & "' where machine_id = '" & m_id & "'"
                sdb.RunCmd(rec_sql, CommandType.Text)
            End If
    ...
    Sub ins_err(ByVal m_id, ByVal p_date) '170428
        ...
        For i As Integer = 0 To 6
            ...
            Dim lb = i.ToString
            ad_101X = ad_101X & lb
            AxActEasyIF5.ReadDeviceBlock(ad_101X, 1, add_D101X(0), AxActUtlType1)
    ...
    Sub day_packing_time(ByVal m_id)
        ...
        For k As Integer = 1 To 2
            If k = 1 Then
                AxActEasyIF5.ReadDeviceBlock("D1499", 1, add_D1499(0), AxActUtlType1)
                p_date_day = add_D1499(0)
            Else
                AxActEasyIF5.ReadDeviceBlock("D1599", 1, add_D1599(0), AxActUtlType1)
            ...
            For i As Integer = 0 To 23
                ...
                AxActEasyIF5.ReadDeviceBlock(pack_num_hour, 1, add_D14XX(0), AxActUtlType1)
                Dim pack_num As String = add_D14XX(0)

                Dim j = i + 50
                power_usage_hour = power_usage_hour & j.ToString
                AxActEasyIF5.ReadDeviceBlock(power_usage_hour, 1, add_D14XX(0), AxActUtlType1)
    ...
    Private Sub Timer2_Tick(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Timer2.Tick
        ...
        While auto_m = 0 And ti < 3
            ...
            'Save_Error("Timer2_Tick 2#start " & ti & Now())
            'AxActEasyIF5.ReadDeviceBlock("D1021", 15, add_D1021(0), AxActUtlType1)
            Try
                AxActUtlType1.ReadDeviceBlock2("D1021", 15, add_D1021(0))
                AxActUtlType1.ReadDeviceBlock2("D1021", 15, add_D1021(0))
                AxActUtlType1.ReadDeviceBlock2(plc_p, 1, add_R5(0))
                AxActUtlType1.ReadDeviceBlock2("R1", 8, add_R1(0))     '171006打包長度設定值
                AxActUtlType1.ReadDeviceBlock2("D54", 3, add_D54(0))   '171006打包長度現在值
                AxActUtlType1.ReadDeviceBlock2("R3", 8, add_R3(0))     '171006打包長度設定值
                AxActUtlType1.ReadDeviceBlock2("D58", 3, add_D58(0))   '171006打包長度現在值
                package_len_set = add_R1(0)
                package_len_real = add_D54(0)
                package_push_set = add_R3(0)
                package_push_real = add_D58(0)
            Catch exception As Exception
                Save_Error("AxActUtlType1 Read error " & Now() & exception.Message)
                'MessageBox.Show(exception.Message, Name, MessageBoxButtons.OK, MessageBoxIcon.Error)
            End Try

            'Save_Error("Timer2_Tick 2#zhon " & ti & Now())
            auto_m = add_D1021(0)
            If auto_m > 0 Then
                'If auto_m.Trim() <> "" Then
                AxActEasyIF5.ReadDeviceBlock(plc_p, 1, add_R5(0), AxActUtlType1)
            Else
                'AxActEasyIF5.Close()
            End If
            'Save_Error("Timer2_Tick 2#end " & ti & Now())
            ti = ti + 1
        ...
        'Dim sssss As Integer = BinaryBit(Integer.Parse(auto_m), 0) '取第几个位置


        If BinaryBit(auto_m, 2) = 1 Then
            Label3.Text = "全自動"
        ElseIf BinaryBit(auto_m, 1) = 1 Then
            Label3.Text = "半自動"
        ElseIf BinaryBit(auto_m, 0) = 1 Then
            Label3.Text = "手動"
        Else
            Label3.Text = "關機中"
        End If
    ...
    Function get_pa_id(ByVal m_id As String, ByVal p_date As Date) As DataTable
        Dim paid_str As String = "select erpid,pkg_len,pkg_push,set_date from pkg_id where plcid = '" & m_id & "' "
        Dim get_paid As DataTable = sdb.GetDataTable(paid_str, CommandType.Text)

        If ((Val(get_paid.Rows(0)(1) & "") <> package_len_set) And package_len_set <> 0) Or _
            ((Val(get_paid.Rows(0)(2) & "") <> package_push_set) And package_push_set <> 0) Then
            Dim p_id_up As String = "UPDATE PKG_ID set pkg_len = '" & package_len_set & "',pkg_push = '" & package_push_set & "',set_date = '" & p_date & "'" & _
                                    " WHERE plcid = '" & m_id & "' "
            sdb.RunCmd(p_id_up, CommandType.Text)
            paid_str = "select erpid,pkg_len,pkg_push,set_date from pkg_id where plcid = '" & m_id & "' "
            get_paid = sdb.GetDataTable(paid_str, CommandType.Text)
            setting_change = True
        End If
        Return get_paid
    End Function
    Sub ins_pac(ByVal m_id, ByVal p_date)
        Dim pa_id As String
        pa_id = get_pa_id(m_id)
        Dim o_len As Decimal = Convert.ToDecimal(get_pa_id(m_id, p_date).Rows(0)(1))
        Dim o_push As Double = Convert.ToDouble(get_pa_id(m_id, p_date).Rows(0)(2))
        Dim set_date As String = Format(get_pa_id(m_id, p_date).Rows(0)(3), "yyyy/MM/dd")
        'Select Case m_id
        '    Case "PK001"
        '        pa_id = "B1"
        '    Case "PK002"
        ...
        Dim g_pakweight As String = "select sum(weighing_tot) from period_packing_detail where a_date = '" & p_date & "' and machine_id = '" & m_id & "'"


        Try
            Dim pakweight As DataTable = sdb.GetDataTable(g_pakweight, CommandType.Text)
            pakw = Val(pakweight.Rows(0)(0) & "")
            If pakw < 1 Then
                ins_pac_sql(m_id, Format(Now(), "yyyy/MM/dd"))
            End If
            ...
            If paknm <> 0 Then
                ...
                Try
                    If Pac_fileBindingSource.Count = 0 Then
                        ...
                        Pac_fileBindingSource.Current("pac11") = o_len
                        Pac_fileBindingSource.Current("pac12") = o_push
                        Pac_fileBindingSource.Current("pac13") = set_date
                        Pac_fileBindingSource.Current("pacgrup") = "system"
                    ...
                    Else
                        ...
                        Pac_fileBindingSource.Current("pac08") = ""
                        Pac_fileBindingSource.Current("pac09") = power_ds
                        If package_len_set <> 0 Then
                            Pac_fileBindingSource.Current("pac11") = package_len_set
                        End If
                        If package_push_set <> 0 Then
                            Pac_fileBindingSource.Current("pac12") = package_push_set
                        End If
                        If pakw <> 0 Then
                            Pac_fileBindingSource.Current("pac10") = pakw
                        End If
                        If setting_change = True Then
                            Pac_fileBindingSource.Current("pac13") = p_date
                            Save_plc_period("各時段打包紀錄 " & "日期: " & Now() & "打包日期: " & p_date & _
                                        "長度:(舊值)" & o_len & " (新值)" & package_len_set & _
                                        " 擠壓壓力:(舊)" & o_push & " (新)" & package_push_set & "")
                    ...
                    Pac_fileTableAdapter.Update(Ebi_db.pac_file)
                Catch exception As Exception
                    Save_Error(Now() & " :pac_file pac 寫入錯誤!" & exception.Message)
                    'MessageBox.Show(exception.Message, Name, MessageBoxButtons.OK, MessageBoxIcon.Error)
                End Try
                Pac_fileBindingSource.Filter = ""
            End If
    ...
    'End Sub
    '将整数转换为2进制字符串，返回某个位的int类型1或0
    Function BinaryBit(ByVal lngValue As String, ByVal lngIndex As Long) As Integer
        BinaryBit = (lngValue \ 2 ^ lngIndex) Mod 2
    End Function
End Class
Class AxActEasyIF5

    Shared Sub ReadDeviceBlock(ByVal a0 As String, ByVal p2 As Integer, ByRef add_R2000 As Integer, ByVal AxActUtlType1 As AxActUtlTypeLib.AxActUtlType)
        AxActUtlType1.ReadDeviceBlock2(a0, p2, add_R2000)

    End Sub

    Shared Function Open() As Boolean
        Return True
    End Function
